...
%DIF DEL /var/folders/7d/qc682nsj13d_bz11bztzd5hm0000gn/T/huGPpft0I2/latexdiff-vc-HEAD/00_Article_Merge.tex   Mon Mar  2 12:59:55 2026
...
Although our model was inspired by the observed distribution of compartment-specific spine tuning in visual cortex, it makes predictions that extend beyond that context. Our results with an abstract latent variable model in \hyperref[fig:4]{Figure 4} shows that reductions in synaptic depression promote weakly correlated synapses in general. While this applies to edge tuning in visual cortex, it may explain other tuning features in visual cortex or different sensory areas. Weakly correlated synapses are a fundamental component to multimodal tuning, in which the synapses across input modes are not as correlated with each other as synapses within an input mode. Vanilla STDP would predict that one input mode would eventually “win out”  \citep{songCompetitiveHebbianLearning2000}. Our model provides an explanation for how multiple input modes can be preserved within a single cell. Furthermore, it predicts that dendritic spines tuned to a secondary input mode, like movement tuning in primary visual cortex, are preferentially observed on branches with reduced synaptic depression \citep{xuNonlinearDendriticIntegration2012,bugeonTranscriptomicAxisPredicts2022}. As above, this is testable by comparing spine tuning properties with local dendritic branch structure.

We prioritized a simple model that is sufficient to explain the observed distribution of spine tuning in visual cortex. However, our model can be extended to include more complex or biophysically realistic plasticity rules. For example, voltage-dependent STDP may specifically interact with morphology of dendrites beyond the effect on AP-evoked calcium influx we modeled here \citep{clopathConnectivityReflectsCoding2010}. Additionally, our model ignores local plasticity mechanisms like dendritic spikes that can produce even more compartment-specific clustering of synaptic input tuning, which may be exaggerated in distal-complex branches \citep{bonoModelingSomaticDendritic2017}.

Together, our modelling shows that compartment-specific plasticity rules determined by the biophysical properties of dendrites can explain the observed distribution of synaptic tuning in visual cortex. Our model makes specific predictions about the branch-specific distribution of retinotopically displaced inputs that can be tested in future experiments. More broadly, our work shows how considering the physical structure of neurons can lead to insights about the computational properties of dendrites. 

\section*{Methods}\label{s:methods}

\subsection*{Experimental Data, Figure 1}
Experimental data was collected as part of a previous study \citep{landauDendriticBranchStructure2022}. We refer the reader to that paper for a detailed description of the experimental methods. Here, we provide a brief summary of the methods relevant to the data in \hyperref[fig:1]{Figure 1}. Data was collected from acute coronal slices of C57Bl/6j wild-type mice between postnatal days P21-P28. Somatic whole-cell recordings were acquired from cortical layer 2/3 pyramidal cells to measure intracellular voltage and elicit APs through brief current injection. We measured calcium influx through two-photon imaging of Fluo-5F and stimulated synaptic input through two-photon glutamate uncaging. In pairing protocols, APs were evoked 5ms after glutamate uncaging. Synthetic linear sums were computed by adding the baseline-subtracted calcium signal evoked by APs or glutamate uncaging alone, with a 5ms offset to reflect the timing of pairing protocols.
...
To generate the transfer functions in \hyperref[fig:2]{Figure 2g, h}, we first simulated AP-evoked calcium influx across a range of AP amplitudes (0--100 mV from a $-70$ mV baseline) and measured integrated calcium for NMDAR and VGCC currents separately. The Nevian/Sakmann curve in \hyperref[fig:2]{Figure 2f} gives plasticity as a function of added buffer concentration, not directly as a function of calcium. To convert this into a transfer function of the form \DIFdelbegin \DIFdel{$Plasticity(relative [Ca]) = f([Ca])$, we assumed buffer-capacity scaling, with $\kappa = [B]/K_d$. }\DIFdelend \DIFaddbegin \DIFadd{$Plasticity = f([Ca]_{relative})$, we used mass action to model relative calcium as a function of added buffer. At steady-state, the relationship between buffer concentration and free calcium concentration follows: $\frac{[CaB]}{[Ca]} = \frac{[B]}{K_d} = \kappa$. Buffer dynamics are fast compared to calcium influx and extrusion, so we can assume steady-state. }\DIFaddend Additionally, we assume that $[B]$ doesn't appreciably change due to calcium binding (which is true given the small changes in calcium concentration \citep{sabatiniLifeCycleCa22002}). \DIFdelbegin \DIFdel{In steady-state, the relationship between buffer concentration and free calcium concentration follows mass action: $\frac{[CaB]}{[Ca]} = \frac{[B]}{K_d} = \kappa$. }\DIFdelend Therefore, we can solve for the fraction of free calcium remaining as a function of added buffer:
...
\begin{equation*}
\tau_m \DIFdelbegin \DIFdel{\frac{\Delta V}{dt} }\DIFdelend \DIFaddbegin \DIFadd{\frac{d}{dt}V }\DIFaddend = (V_{\mathrm{rest}} - V) + R_m I_{\mathrm{syn}},
\end{equation*}
...
\begin{equation*}
    \DIFdelbegin \DIFdel{\Delta g}\DIFdelend \DIFaddbegin \DIFadd{\frac{d}{dt}G}\DIFaddend _k = \sum\DIFdelbegin \DIFdel{_j }\DIFdelend \DIFaddbegin \DIFadd{_i }\DIFaddend s\DIFdelbegin \DIFdel{_j}\DIFdelend \DIFaddbegin \DIFadd{_i}\DIFaddend \,w\DIFdelbegin \DIFdel{_j}\DIFdelend \DIFaddbegin \DIFadd{_i}\DIFaddend \, \DIFdelbegin \DIFdel{\mathbf{1}[w_j>g_{\mathrm{th}}] }\DIFdelend - \DIFdelbegin \DIFdel{g_k \frac{\Delta t}{\tau_k}
}\DIFdelend \DIFaddbegin \DIFadd{\frac{G_k}{\tau_k}
}\DIFaddend \end{equation*}
\DIFdelbegin \DIFdel{where $s_j(t)\in\{0,1\}$ }\DIFdelend \DIFaddbegin 

\DIFadd{where $s_i(t)\in\{0,1\}$ }\DIFaddend is a presynaptic spike and \DIFdelbegin \DIFdel{$w_j$ }\DIFdelend \DIFaddbegin \DIFadd{$w_i$ }\DIFaddend is synaptic weight. \DIFdelbegin \DIFdel{We used a fixed conductance threshold below which synapses }\DIFdelend \DIFaddbegin \DIFadd{Any synapse whos synaptic weight was below a fixed threshold $w_{threshold}$ }\DIFaddend did not contribute to \DIFdelbegin \DIFdel{the synaptic current
, denoted $g_{\mathrm{th}}$.
}\DIFdelend \DIFaddbegin \DIFadd{synaptic current
}\DIFaddend 

\DIFaddbegin \begin{equation*}
\DIFadd{effective(w_i) = \begin{cases}
0 & \text{if } w_i < w_{threshold} \\
w_i & \text{otherwise}
\end{cases}
}\end{equation*}

\DIFaddend We used additive STDP with separate potentiation and depression eligibility traces. $A_+$ and $A_-$ are the potentiation and depression increments, respectively, which represent the maximum change in synaptic weight by a STDP pairing. Let
\begin{equation*}
...
\DIFdelbegin \DIFdel{Whenever a presynaptic spike occurs, we }\DIFdelend \DIFaddbegin \DIFadd{STDP is implemented in three update rules. }\DIFaddend (1) \DIFdelbegin \DIFdel{reduce the synaptic weight in proportion to the global eligibility trace for synaptic depression (set by postsynaptic spikes, $D$) , }\DIFdelend \DIFaddbegin \DIFadd{Each neuron maintains a global depression trace that increments upon each postsynaptic spike ($s_{post}$) and has exponential decay with time constant $\tau_-$. }\DIFaddend (2) \DIFdelbegin \DIFdel{increase the potentiation eligibility trace , $P_j$, of each synapse by $A_+$, and }\DIFdelend \DIFaddbegin \DIFadd{Each synapse maintains a potentiation trace that increments upon each presynaptic spike and has exponential decay with time constant $\tau_+$. }\DIFaddend (3) \DIFdelbegin \DIFdel{decay both the potentiation and depression eligibility traces by their respective time constants. The eligibility traces are initialized to 0 at the start of the simulation.
}\begin{align*}
    \DIFdel{\Delta w_j }&\DIFdel{= s_j D }\\
    \DIFdel{\Delta P_j }&\DIFdel{= s_j A_+ - \frac{\Delta t}{\tau_+}P_j }\\
    \DIFdel{\Delta D }&\DIFdel{= - \frac{\Delta t}{\tau_-}D
}\end{align*}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Whenever a postsynaptic }\DIFdelend \DIFaddbegin \DIFadd{Each synapse's weight is updated by the depression trace whenever a presynaptic }\DIFaddend spike occurs, \DIFdelbegin \DIFdel{we (1) increment the depression eligibility trace by $A_-$, and (2) update the synaptic weight in proportion to the potentiation eligibility trace .
}\begin{align*}
\DIFdel{\Delta D }&\DIFdel{= A_-, }\\
\DIFdel{\Delta w_j }&\DIFdel{= P_j.
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{and by the potentiation trace whenever a postsynaptic spike occurs. }\DIFaddend Weights are clipped to $[0,w_{\max}]$.

\DIFaddbegin \begin{align*}
    \DIFadd{\frac{d}{dt}w_i }&\DIFadd{= P_i \, s_{post} - s_i D}\\
    \DIFadd{\frac{d}{dt}P_i }&\DIFadd{= s_i A_+ - \frac{P_i}{\tau_+} }\\
    \DIFadd{\frac{d}{dt}D }&\DIFadd{= A_- \, s_{post} - \frac{D}{\tau_-}
}\end{align*}

\DIFaddend To stabilize firing rates, we included a multiplicative homeostatic plasticity rule, inspired by \citep{turrigianoHomeostaticPlasticityDeveloping2004}. Neurons had a set point firing rate, $r_{\mathrm{set}}$ and a dynamic firing rate estimate $\hat r$ that evolved slowly over time (much slower than the time step and STDP timescales) \DIFdelbegin \DIFdel{. The firing rate estimate was updated as
}\begin{displaymath}
\DIFdel{\Delta \hat r = \frac{1}{\tau_h}\left(spike - \frac{\Delta\hat r}{dt}\right),
}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The homeostatic instructive signal was then computed as }\begin{displaymath}
    \DIFdel{h_{inst}=\log\!\left(\frac{r_{\mathrm{set}}}{\hat r}\right),
}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{with exponential decay and incremented by the instantaneous firing rate ($r_{inst}$, where $\Delta t$ is the time step). The homeostatic drive was computed as the log ratio of the rate estimate and the set point, and }\DIFaddend synapses were multiplicatively \DIFdelbegin \DIFdel{scaled by }\begin{displaymath}
    \DIFdel{\Delta w_j = h_{inst} \, w_j.
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{updated by the homeostatic drive. Both equations use $\tau_h$ which is much slower than all other time constants used in the model (20 seconds).
}

\begin{align*}
    \DIFadd{\tau_h \frac{d}{dt}\hat r }&\DIFadd{= r_{inst} - \hat r }\\
    \DIFadd{r_{inst} }&\DIFadd{= s_{post} / \Delta t }\\
    \DIFadd{h_{drive} }&\DIFadd{= \log\!\left(\frac{r_{\mathrm{set}}}{\hat r}\right), }\\
    \DIFadd{\tau_h \frac{d}{dt} w_i }&\DIFadd{= h_{drive} \, w_i 
}\end{align*}\DIFaddend 
