...
from .build import build_iaf
from .step import step_iaf


def run_iaf(ap_dep_idx: int, num_state_idx: int, basal_dep_follow: int, T: int = 5000 * 1000) -> Tuple[IafNeuron, List[int], np.ndarray, np.ndarray]:
    ...
    T = 5000 * 1000  # model stabilizes around T=10000*1000
    ...
    if options['sourceMethod'] == 'divide':
        ...
        source_loading = np.array([np.roll(gauss_loading, signal * shift_input_per_signal) 
                                 for signal in range(options['numSignals'])])
    ...
    small_basal_weight = np.zeros((options['numInputs'], T // 1000))
    small_apical_weight = np.zeros((options['numInputs'], T // 1000))
    
    # Build the model
    iaf = build_iaf(options)
    
    # Run simulation
    need_input = True
    y = []  # keep track of input values
    y_change = []  # keep track of when input changes
    
    for t in tqdm(range(T)):
        # Generate Input
        if need_input:
            interval = int(np.random.exponential(tau_stim)) + 1
            input_vec = (np.random.randn(options['numInputs']) + 
                        np.random.randn(options['numSignals']).dot(options['sourceLoading'])) / options['varAdjustment']
        ...
        if t % 1000 == 0:
            for input_idx in range(iaf.numInputs):
                small_basal_weight[input_idx, t//1000] = np.sum(
                    iaf.basalWeight[iaf.basalTuneIdx == input_idx])
                small_apical_weight[input_idx, t//1000] = np.sum(
                    iaf.apicalWeight[iaf.apicalTuneIdx == input_idx])
    
    spk_times = np.where(spikes)[0]
    
    return iaf, spk_times, small_basal_weight, small_apical_weight