function iaf = step_cba(iaf,inputOri)
...
basalRate = iaf.getRateBasal(inputOri,iaf.basalTuneCenter);
...
apicalRate = iaf.getRateApical(inputOri,iaf.apicalTuneCenter,iaf.apicalTuneIdx);
...
iaf.basalTuneCenter(replaceSynapse) = iaf.angles(randi(iaf.numAngles,sum(replaceSynapse),1));
...
iaf.apicalTuneIdx(replaceSynapse) = iaf.apicalIndices(randi(length(iaf.apicalIndices),sum(replaceSynapse),1));
iaf.apicalTuneCenter(replaceSynapse) = iaf.angles(randi(iaf.numAngles,sum(replaceSynapse),1));
