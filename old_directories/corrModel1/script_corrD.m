...
numStateIdx = 1;
basalFollow = 0;
[iaf,spkTimes,smallBasalWeight,smallApicalWeight] = runo2_corrD(1,apDepIdx,numStateIdx,basalFollow);
...
sbw = filter(ones(1,smoothFactor),1,smallBasalWeight(:,end));
saw = filter(ones(1,smoothFactor),1,smallApicalWeight(:,end));
plot(sbw/max(sbw),iaf.numInputs:-1:1,'color','k','linewidth',1.5);
plot(saw/max(saw),iaf.numInputs:-1:1,'color','b','linewidth',1.5);

smTrajectory = 50;
figure(3); clf; 
set(gcf,'units','normalized','outerposition',[0.61 0.63 0.22 0.34]);
subplot(1,2,1);
plot(1:size(smallBasalWeight,2), filter(ones(1,smTrajectory),1,smallBasalWeight,[],2));
subplot(1,2,2);
plot(1:size(smallApicalWeight,2), filter(ones(1,smTrajectory),1,smallApicalWeight,[],2));
%% -- analyze data -- 
