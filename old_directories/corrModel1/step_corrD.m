function iaf = step_corrD(iaf,inputRates)
...
basalRate = inputRates(iaf.basalTuneIdx); % find presynaptic rate for each basal synapses
...
apicalRate = inputRates(iaf.apicalTuneIdx); % find presynaptic rate for each apical synapses
...
% Generate Inhibitory Conductances (basically just some noise)
...
iaf.basalWeight = iaf.basalWeight + basalSpikes .* iaf.basalDepression; % depress weights
basalReplace = iaf.basalWeight < iaf.minBasalWeight; % index of synapses to be replaced
iaf.basalTuneIdx(basalReplace) = randi(iaf.numInputs,sum(basalReplace),1); % randomly change presynaptic input connection
iaf.basalWeight(basalReplace) = iaf.basalStartWeight; % update replacement start weights
...
% Do Homeostasis
iaf.homRateEstimate = iaf.homRateEstimate + iaf.dt/iaf.homTau * (1*iaf.spike/iaf.dt - iaf.homRateEstimate);  % update rate estimate
iaf.homScale = iaf.homRate / max(0.1,iaf.homRateEstimate) - 1; % fraction change (positive or negative), use max(0.1, x) to prevent blowup
