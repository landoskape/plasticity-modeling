...
class SynapseGroup:
    ...
    lose_synapse_ratio: float
        The ratio of synapses to lose during homeostasis.
    new_synapse_ratio: float
        The ratio of synapses to create during homeostasis.
    conductance_threshold: float
        The conductance threshold for the synapse group.
    """

    def __init__(
        self,
        num_synapses: int,
        max_weight: float,  # in relative units of conductance
        reversal: float,  # in V
        ...
        conductance_threshold: float = 0.1,  # in relative units of max_weight
        ...
        self.tau: float = tau
        self.dt: float = dt
        self.rng = create_rng()  # biophysically plausible random number generator ("haha")
        ...
        if use_replacement:
            ...
            self.min_conductance: float = self.max_weight * conductance_threshold
    ...
    def time_step(self, input_rates: np.ndarray, homeostasis: float | None = None):
        ...
        new_conductance = compute_conductance(spikes, self.weights, self.conductance_threshold)
        ...
        if self.use_stdp:
            ...
            self.depression_eligibility -= self.depression_increment * self._dt_depression_tau
